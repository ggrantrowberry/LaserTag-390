...
queue_index_t incrementIndex(queue_t* q, queue_index_t index){
    ...
    //Checks to see if the index is at the end
    if(index == q->size - INDEX_FIX){
...
bool queue_full(queue_t* q){
    ...
    //Makes an index
    queue_index_t index = incrementIndex(q, q->indexIn);
    
    //Checks to see if indexIn incremented is equal to indexOut
    if(index == q->indexOut){
...
bool queue_empty(queue_t* q){
    ...
    //Checks to see if the indexIn is equal to indexOut
    ...
    //Return that the queue is not empty
    return false;
}
...
void queue_push(queue_t* q, queue_data_t value){
    ...
    if(!queue_full(q)){
        ...
        //Increments indexIn
        q->indexIn = incrementIndex(q, q->indexIn);
        
        //Sets the underflowFlag to false
        q->underflowFlag = false;
        
        //Increments the elementCount
        q->elementCount = q->elementCount + INCREMENT_ONE;
        
    } else {
        
        //Sets the overflowFlag
        q->overflowFlag = true;
        
        //Prints
        printf("Queue Push Failed. The Queue Is Full\n\r");
...
queue_data_t queue_pop(queue_t* q){
    ...
    //Makes a spot for oldest element in queue
    queue_data_t oldestElem;
    
    //Checks to see if the queue is empty
    if(!queue_empty(q)){
        
        //Sets the oldest element
        oldestElem =  q->data[q->indexOut];
        
        //Increments indexOut
        q->indexOut = incrementIndex(q, q->indexOut);
        
        //Decrements the elementCount
        q->elementCount = q->elementCount - INCREMENT_ONE;
        
        //Clears queueOverflowFlag
        ...
        //Sets the underflowFlag
        q->underflowFlag = true;
        
        //Prints error
        printf("Error. Tried to access an empty queue.\n\r");
        
        //Returns 0. Doesn't really matter though.
        return INIT_VALUE;
...
queue_data_t queue_readElementAt(queue_t* q, queue_index_t index){
    ...
    if(index > q->elementCount - INDEX_FIX){
